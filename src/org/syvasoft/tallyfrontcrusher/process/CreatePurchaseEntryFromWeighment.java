			...
			try {
				if(!createConsolidatedPurchaseInvoice) {
					...
					if(!pprice.isRentInclusive() && ord.getTF_RentedVehicle_ID() > 0) {
						...
						BigDecimal RentAmt=MLumpSumRentConfig.getLumpSumRent(getCtx(),ord.getAD_Org_ID(),rv.getC_BPartner_ID(),ord.getC_BPartner_ID(),ord.getItem1_ID() ,ord.getTF_Destination_ID(), 
								ord.getItem1_VehicleType_ID(), dest.getDistance(), "");
						ord.setRent_Amt(RentAmt);
						ord.setRentMargin(BigDecimal.ZERO);
						ord.setRentPayable(RentAmt);
