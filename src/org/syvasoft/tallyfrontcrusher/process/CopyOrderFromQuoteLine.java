 ...
 */
public class CopyOrderFromQuoteLine extends SvrProcess
{
	/** Order to Copy				*/
	...
	{
		if (log.isLoggable(Level.INFO)) log.info("C_OrderLine_ID=" + RecordId 
			+ ", C_DocType_ID=" + p_C_DocType_ID 
			+ ", CloseDocument=" + p_IsCloseDocument);
		if (RecordId == 0)
			throw new IllegalArgumentException("No Order");
		
		MOrderLine fromLine = new MOrderLine (getCtx(), RecordId, get_TrxName());
		...
		//
		MOrder from = new MOrder (getCtx(), fromLine.getC_Order_ID(), get_TrxName());
		...
		if(from.getDocStatus().equals(MOrder.DOCSTATUS_Completed)) {
			...
			{
				MOrder original = new MOrder (getCtx(), fromLine.getC_Order_ID(), get_TrxName());
		...
		//	return "@C_Order_ID@ " + newOrder.getDocumentNo();
			...
			throw new AdempiereException("Error: Quotation Status should be completed before creating Order!");
		}
