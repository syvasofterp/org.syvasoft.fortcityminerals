...
public class RequisitionPOCreate extends SvrProcess
{
	...
	/** Order				*/
	private TF_MOrder		m_order = null;
	/** Order Line			*/
	private TF_MOrderLine	m_orderLine = null;
	/** Orders Cache : (C_BPartner_ID, DateRequired, M_PriceList_ID) -> MOrder */
	private HashMap<MultiKey, TF_MOrder> m_cacheOrders = new HashMap<MultiKey, TF_MOrder>();
	...
	private void newOrder(MRequisitionLine rLine, int C_BPartner_ID) throws Exception
	{
		...
		if (m_order == null)
		{
			m_order = new TF_MOrder(getCtx(), 0, get_TrxName());
	...
	private void newLine(MRequisitionLine rLine) throws Exception
	{
		...
		//	No Order Line
		m_orderLine = new TF_MOrderLine(m_order);
		...
		if (product != null)
		{
			...
			if(priceUOM != null){
				...
				m_orderLine.setPrice(price);
				
				m_orderLine.setLineNetAmt(price.multiply(rLine.getQty()).divide(BigDecimal.ONE, 2, RoundingMode.HALF_UP));
			}
			else{
				m_orderLine.setPrice(BigDecimal.ZERO);
				
				m_orderLine.setLineNetAmt(BigDecimal.ZERO);
			}
		}
		else
		{
